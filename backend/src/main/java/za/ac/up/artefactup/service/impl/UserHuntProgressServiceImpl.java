...
import za.ac.up.artefactup.aspect.ValidateInputUserProgress;
...
public class UserHuntProgressServiceImpl implements UserHuntProgressService<UserHuntProgress> {
    ...
    public UserHuntProgress getUserProgress(final Long huntId, final String sessionId) {
        Optional<UserHuntProgress> userHuntProgress = userHuntProgressRepository.findBySessionIdAndHuntId(sessionId,
                huntId);
        if (userHuntProgress.isPresent()) {
            ...
            throw new NotFoundException(
                    "UserHuntProgress not found for sessionId: " + sessionId + " and huntId: " + huntId);
        }
    }
    ...
    public UserHuntStatsDTO getUserHuntStats(final String sessionId) {
        ...
        int artefactsFound = Math
                .toIntExact(Optional.ofNullable(userHuntProgressRepository.sumOfArtefactsFound(sessionId)).orElse(0L));
        int totalArtefacts = (int) scavengerHuntStepRepository.count();
        int totalHunts = scavengerHuntService.count();
        List<Integer> attemptedHuntIds = userHuntProgressRepository.findAllBySessionId(sessionId).stream()
