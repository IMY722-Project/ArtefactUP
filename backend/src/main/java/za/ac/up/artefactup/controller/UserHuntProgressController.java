...
public class UserHuntProgressController {
    ...
    @GetMapping("{huntId}")
    public ResponseEntity<UserHuntProgressDTO> getUserProgress(@PathVariable Long huntId,
            @RequestHeader("Session-id") String sessionId) {
        return ResponseEntity.ok(userHuntProgressFacade.getUserProgress(huntId, sessionId));
    }

    @PostMapping("/start/{huntId}")
    public ResponseEntity<UserHuntProgressDTO> startHunt(@PathVariable Long huntId,
            @RequestHeader("Session-id") String sessionId) {
